...
1. Search the [issues](https://github.com/zscaler/terraform-modules-zscaler-ci-workflows/issues) to see if there is an existing issue. If not, open an issue (note the issue ID).

1. Fork the repository to your personal namespace (only need to do this once).

1. Clone the repo from your personal namespace.

   `git clone https://github.com/{username}/terraform-modules-zscaler-ci-workflows.git`
   Ensure that `{username}` is _your_ user name.

1. Add the source repository as an upsteam.

   `git remote add upstream https://github.com/zscaler/terraform-modules-zscaler-ci-workflows.git`

1. Create a branch which corresponds to the issue ID created in step 1.

   For example, if the issue ID is 101:
   `git checkout -b 101-updating-app-connector-group-templates`
...
1. Push changes to _your_ repository
   ...
   `git push origin/101-updating-app-connector-group-templates`
...
1. Once the PR has been merged, delete your local branch
   ...
   `git branch -D 101-updating-app-connector-group-templates`

### Existing Contributors

1. Search the [issues](https://github.com/zscaler/terraform-modules-zscaler-ci-workflows.git/issues) to see if there is an existing issue. If not, open an issue (note the issue ID).
    ...
    Example:
        ...
        <!-- language: yaml -->
            ...
            jobs:
              pr_ci_wrkflw:
                name: Run CI
                uses: {username}/terraform-modules-zscaler-ci-workflows/.github/workflows/pr_ci.yml@{branchname}
